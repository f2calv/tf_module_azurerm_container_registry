...
- Before adding a `+semver:` directive, inspect every commit between the merge base with `origin/main` and `HEAD`. The branch must contain exactly one `+semver:` directive; if one already exists, do not add another.
...
- Place the directive in the commit that introduces the versioned behavior. Do not repeat it in documentation, formatting or release-reference synchronization commits.
- After all branch changes are complete, run GitVersion with the repository's `GitVersion.yml` and verify the final numeric major, minor and patch before creating a pull request.
- Publish reusable modules with immutable plain `X.Y.Z` tags. Final releases on `main` must not use a `v` prefix, moving aliases, CI build metadata or pre-release suffixes.
- Reusable release-versioning workflow calls must set `tag-prefix: ''` and `move-major-tag: false`.
- Pin the module source example in `README.md` to the final immutable `X.Y.Z` tag expected from the current change, not `main`, a moving alias, a feature-branch pre-release or the previous release.
- Treat a mismatch between the README source ref and the expected final GitVersion tag as a CI failure.
